...
| **Synology Photos AI 기능(얼굴인식/인물클러스터링) 끄기** | 중~큼 | ✅ **조치 완료(2026-09-08)** — 아래 참고 |
...
### ✅ 조치 완료: Synology Photos AI 기능 비활성화 (2026-09-08)
사용자 확인 결과 얼굴인식/인물클러스터링은 사용하지 않고, 검색 기능(족보 등 파일 찾기 용도)만 필요하다고 하여 AI 기능만 선별적으로 껐음:

1. Photos 자체 DB(PostgreSQL, `synofoto`)의 `config` 테이블에서 **`enable_person`**이 이 기능의 마스터 스위치임을 확인(`psql -h /run/synofoto -p 5432 -U SynologyPhotos synofoto`로 접속)
2. `UPDATE config SET value='false' WHERE key='enable_person';` 실행
3. `synopkg restart SynologyPhotos`로 패키지를 정상적으로 재시작해 변경사항을 반영(데몬을 강제 종료하지 않고 정식 재시작 절차로 처리)
4. 재시작 후 확인: `synofoto-face-extraction`/`synofoto-person-clustering` 데몬이 CPU 사용량 상위 목록에서 사라짐(존재는 하지만 유휴 상태)
5. **파일 이름/내용 검색(SynoFinder, `fileindexd`)은 건드리지 않았음** — 족보 파일 찾기 등 실제 사용 중인 검색 기능에는 영향 없음

재시작 직후 load average가 일시적으로 9~11까지 치솟았으나, 확인해보니 **원인은 Photos가 아니라 그 순간 실제 웹사이트(php-fpm) 트래픽이 몰린 것**이었음(php-fpm 워커 9개가 동시에 33~50% CPU 사용) — 위에서 진단한 "PHP 워커가 몰리면 4코어가 꽉 찬다"는 근본 문제를 실시간으로 재확인한 셈이며, 이번 조치와는 무관함.
...
### ✅ 조치 완료 (2026-09-07)
사용자 동의 하에 Gmail 앱 비밀번호로 전환하는 방식으로 즉시 수정함:

1. `/usr/bin/ssmtp`(기존 정체불명 OAuth2 바이너리)를 `/usr/bin/ssmtp.oauth2-broken.bak`으로 백업 후 보존(롤백 가능)
...
- Rhymix로 완전히 이전하면 관리자 화면에서 SMTP를 다시 설정하게 되므로(레거시 `useSMTP()`/`useGmailAccount()`와 동일 계열 기능), 그 시점에 전용 서비스로 정리하는 걸 권장
- 원본 백업(`ssmtp.oauth2-broken.bak`)은 삭제하지 않고 남겨둠
...
| 문제 | 원인 | 상태 |
|---|---|---|
| CPU 99%/다운 | 1.8GB RAM 기기에 SynoFinder·Photos AI·Drive·LDAP·백업·AV 등 무거운 패키지 과밀 설치, 상시 스왑 45% 사용 | 진단 완료, **부분 조치**(Photos AI 기능 비활성화 완료, 나머지는 미조치 — 근본은 하드웨어 한계) |
| 권한 구조 혼란 | DSM/LDAP(파일 서비스)와 XE(홈페이지)가 완전히 분리된 신원 체계, ACL에 서비스 계정까지 뒤섞여 있음 | 진단 완료, 미조치(원칙 확정+ACL 정리 필요) |
| 메일 미전송 | 개인 Gmail OAuth2 토큰 만료, 갱신 메커니즘 없음(구조적 고장) | **✅ 조치 완료** — 앱 비밀번호 기반 Python3 릴레이로 교체, 테스트 발송 성공 |
