...
| 작품전시회 갤러리 커스텀 스킨 + 승인 워크플로우(검토중→승인→공개 상태전이) | 중~상 | 게시판 자체는 만들어져 있음, board-feature-specs.md §5. ⚠️ 과제게시판에서 상담기능이 부적합했던 것과 비슷하게, 승인 전/후 가시성 전환도 board 표준기능만으로 될지 재검토 필요 — 안 되면 이것도 별도 모듈 전환 검토 |
...
2. ~~자료실~~ → ~~과제 대시보드~~ → **작품전시회 갤러리** 순서로 커스텀 모듈 계속 개발 (남은 건 이거 하나. 다만 board 표준기능 재검토 먼저 필요 — 위 표 참고)
