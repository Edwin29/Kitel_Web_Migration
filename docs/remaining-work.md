...
캘린더([custom_modules/calendar](../custom_modules/calendar))와 자료실([custom_modules/archive](../custom_modules/archive)) 완성. 같은 패턴으로 만들면 되는 나머지:

| 항목 | 난이도 | 참고 |
|---|---|---|
| ~~자료실 커스텀 모듈~~ | ~~중~~ | **완료** — board-feature-specs.md §3 |
| 과제게시판 제출현황 대시보드 (과제×준회원 매트릭스) | 중 | 게시판 자체(상담기능 포함)는 이미 만들어져 있음, board-feature-specs.md §4 |
| 작품전시회 갤러리 커스텀 스킨 + 승인 워크플로우(검토중→승인→공개 상태전이) | 중~상 | 게시판 자체는 만들어져 있음, board-feature-specs.md §5 |
...
2. ~~자료실~~ → **과제 대시보드 → 작품전시회 갤러리** 순서로 커스텀 모듈 계속 개발 (캘린더·자료실과 동일 패턴, 학습 곡선 이미 넘음)
3. 위 항목들 개발하면서 걸리는 **§3 열린 질문들을 그때그때 확정**
4. 커스텀 개발이 어느 정도 끝나면 **데이터 마이그레이션 스크립트** 설계 착수 (그 전에 하면 게시판 구조 변경 시 재작업 위험)
