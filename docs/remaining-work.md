...
캘린더는 완성([custom_modules/calendar](../custom_modules/calendar)). 같은 패턴으로 만들면 되는 나머지:

| 항목 | 난이도 | 참고 |
|---|---|---|
| 자료실 커스텀 모듈 (폴더 트리 탐색 + 업로드/다운로드) | 중 | board-feature-specs.md §3 |
...
2. **자료실 → 과제 대시보드 → 작품전시회 갤러리** 순서로 커스텀 모듈 계속 개발 (캘린더와 동일 패턴, 학습 곡선 이미 넘음)
3. 위 3개 개발하면서 걸리는 **§3 열린 질문들을 그때그때 확정**
