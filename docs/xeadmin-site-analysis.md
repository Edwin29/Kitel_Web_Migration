...
- 사이트: KITEL 동아리 홈페이지 (`kitel.kw.ac.kr`, 광운대학교)
...
└── xe/                    # XE 코어 + 모듈/레이아웃/애드온 + 커스텀 앱 — 실제 운영 소스
    ├── modules/           # 표준 XE 모듈만 존재 (board, comment, member, document 등)
    ...
    └── rental_dev/        # 대여 시스템 개발 소스 (아래 4번 참고)
```
...
## 4. 커스텀 확장: 기자재 대여 시스템 (`xe/rental_dev`)
...
| `faceOff/` | 228K | XE Layout 모듈의 **레이아웃 편집기(FaceOff) 미리보기 캐시** — `modules/layout`의 `faceoff.js` 기능과 연결된 언어별 캐시(`{module_srl}/{lang}.cache.php`). 정상적인 XE 코어 캐시로 확인됨 (이전에 "용도 불명"으로 남겼던 항목, 확인 완료) |
...
5. 디자이너와 함께 신규 IA(정보구조)/디자인 확정 — 위 게시판 목록을 기준으로 유지/통폐합할 게시판 결정
