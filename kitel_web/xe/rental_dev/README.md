...
| 테이블 | 요점 |
|---|---|
| `categories` | 기자재 분류. `tracking_mode`가 `unique`(개별)면 물건마다 행 하나, `bulk`(개수)면 화면에는 개수만. `max_per_user`로 1인당 대여 한도. `next_serial`은 다음 라벨 번호 |
...
- **정책 기본값**
  - 반납 예정일: 대여 시점 +7일 (`default_due_days`).
